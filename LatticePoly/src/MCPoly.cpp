...
void MCPoly::GenerateCAR()
{
	...
	std::mt19937 gen(rd());
	std::vector<double>  weights = {0.00148905, 0.00091369, 0.00134444, 0.00129954, 0.00094887,
	...
	for ( int i = 0; i < (int) weights.size(); ++i )
		weights.at(i)=exp(weights.at(i));
	std::discrete_distribution<> d(weights.begin(), weights.end());
	
	std::vector<int> CAR_sample;
	
	while((int) CAR_sample.size() < int(Nchain*1.25/8))
	{
