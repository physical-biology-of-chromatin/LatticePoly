...
class MonomerDmap():
	def __init__(self, outputDir, initFrame):
		...
		self.readers = []
		self.N_chain=[]
		shuffled_polymers=np.random.permutation(self.polymers)

		for P in shuffled_polymers:
			self.readers.append(vtkReader(outputDir, P, initFrame=initFrame, readLiq=False, readPoly=True ))
	
		self.N_chain=np.sum([(self.readers[0].status==-1)+(self.readers[0].status==0)])*np.ones(len(self.readers),dtype=int)
		self.contactFile = os.path.join(outputDir,"r_"+str(r)+"_"+str(initFrame)+ "_"+str(interval)+"_full_genome.cool")
...
#self.contactProb=np.rint(self.contactProb/finalFrame)
	...
	def Print(self):
		...
		chrom_names=self.polymers
		chromsizes=pd.Series(self.N_chain*1000)
