...
CURRENTNAME=" r_1.0_G1_hic.res"
# sbatch variables
QVARS="SCRATCHDIR=${SCRATCHDIR},SCRIPTDIR=${SCRIPTDIR},CURRENTDIR=${CURRENTDIR},CURRENTNAME=${CURRENTNAME}"

# Check input parameters and submit
if [ "$#" -eq "2" ]; then
sbatch ${QARGS} --export=${QVARS} ${SCRIPTDIR}/merge_matrix.sh
else
