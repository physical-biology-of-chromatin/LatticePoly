...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "\033[1;31mUsage is %s experience program1 data1 program2 data2 ...\033[0m"
            % sys.argv[0]
        )
        sys.exit()
    else:
    ...
    for tmp_experience in os.listdir(XnfsDir):
        if tmp_experience.split("_")[0] == f"EXP{experience}":
            ...
            inputDir_format = inputDir.replace("/", "_")
            verif += 1
...
subprocess.run(
    f"chmod u+x resources/submission/tmp/which_python_process_{JOBNAME}.py",
    shell=True,
    executable="/bin/bash",
)
...
with open(f"resources/submission/tmp/slurm_sweep_process_{JOBNAME}.sh", "w") as file:
    ...
    file.write(
        '\t# Create INDIR/out directory if necessary\n\t[ ! -d "${INDIR}/out" ] && mkdir -p ${INDIR}/out\n\n'
    )

    file.write("\n\t# Perform processing analysis\n")

    file.write(
        f"\tsrun -n1 resources/submission/tmp/which_python_process_{JOBNAME}.py"
        + " ${INDIR}/ "
        + string_param_to_python
        + ">> ${INDIR}/out/"
        + f"process_{JOBNAME}_"
    ...
    file.write(
        "mv ${SLURM_SUBMIT_DIR}/tmp/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out ${ERRORDIR}/process_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.out\n"
    )
    file.write(
        "mv ${SLURM_SUBMIT_DIR}/tmp/${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err ${ERRORDIR}/process_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}.err\n"
    )
