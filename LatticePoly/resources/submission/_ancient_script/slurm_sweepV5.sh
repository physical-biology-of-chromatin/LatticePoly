...
TMPDIR=${PARAM}_${VAL}
[ ! -z "${PARAM2}" ] && TMPDIR=${PARAM2}_${VAL2}_${TMPDIR}_EV_${EV}_LDENS_${LDENS}_VALJLL_${VALJLL}_VALJLP_${VALJLP}_VALJLPP_${VALJLPP}
...
SCRDIR=${PARAM}/${VAL}
[ ! -z "${PARAM2}" ] && SCRDIR=${PARAM2}/${VAL2}/${SCRDIR}/EV/${EV}/LDENS/${LDENS}/VALJLL/${VALJLL}/VALJLP/${VALJLP}/VALJLPP/${VALJLPP}
...
DATDIR=${PARAM}/${VAL}
[ ! -z "${PARAM2}" ] && DATDIR=${PARAM2}/${VAL2}/${DATDIR}/EV/${EV}/LDENS/${LDENS}/VALJLL/${VALJLL}/VALJLP/${VALJLP}/VALJLPP/${VALJLPP}
...
VALJLLSUB="s|\(Valency_Jll[[:space:]]*=[[:space:]]*\)\(.*;\)|\1${VALJLL} ;|;"
VALJLPSUB="s|\(Valency_Jlp[[:space:]]*=[[:space:]]*\)\(.*;\)|\1${VALJLP} ;|;"
VALJLPPSUB="s|\(Valency_Jlpp[[:space:]]*=[[:space:]]*\)\(.*;\)|\1${VALJLPP} ;|;"

# Copy input configuration file to output directory, substituting paths and parameter values
sed -e "${DIRSUB}""${VALSUB}""${VAL2SUB}""${EVSUB}""${LDENSSUB}""${VALJLLSUB}""${VALJLPSUB}""${VALJLPPSUB}" < data/input.cfg > ${TMPDIR}/input.cfg
