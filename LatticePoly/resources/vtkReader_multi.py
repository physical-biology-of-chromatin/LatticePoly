...
class vtkReader():
	...
	def __init__(self, outputDir, n_chrom, initFrame=-1, readLiq=False, readPoly=True, backInBox=False):
		self.outputDir = os.path.realpath(outputDir)
		self.n_chrom=n_chrom
		if os.path.exists(self.outputDir):
			self._liqFile  = os.path.join(self.outputDir, "liq%05d.vtp")
			self._polyFile = os.path.join(self.outputDir, str(self.n_chrom)+"poly%05d.vtp")
	...
	def _readPolyFrame(self):
		...
		try:
			...
			self.Cohesin = vn.vtk_to_numpy(polyData.GetPointData().GetArray("Cohesin"))
	...
	def _parseFileSeqs(self):
		if self._readPoly:
			try:
				polySeq = findSequenceOnDisk(self.outputDir + '/'+str(self.n_chrom)+'poly@.vtp')
