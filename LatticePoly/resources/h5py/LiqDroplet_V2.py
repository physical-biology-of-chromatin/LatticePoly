...
class LifeTime():
        def __init__(self, inputDir):
                ...
                with h5py.File(self.processPath, 'r') as processFile:
                        ...
                        print(np.shape(processFile["liqDropInfo"]), processFile["liqDropInfo"][0])

                        self.liqInfo = np.asarray(cast(h5py.Dataset, processFile["liqInfo"]), dtype = np.int32)
                        self.dropInfo = np.asarray(cast(h5py.Dataset, processFile["dropInfo"]), dtype = [('size', 'i4'),
                               ('center_of_mass', 'f4', (3,)),
                               ('mean_number_of_neighbors', 'f4'),
                               ('r_gyr', 'f4'), ('aniso', 'f4')])
                        
                self.N, self.nLiq, _ = np.shape(self.liqInfo)

                self.dropSizeMax = np.max(self.dropInfo['size'])
                self.dropSizeMin = np.min(np.where(self.dropInfo['size'] > 0, self.dropInfo['size'], 2*self.nLiq))
        ...
        def Compute_graph(self):
                ...
                for frame in range(1, self.N):
                        ...
                        if nLiq_gaz > .5:
                                self.G.add_node((frame, int(-1)), size = nLiq_gaz, droplet = -1, center_of_mass_x = -1., center_of_mass_y = -1., center_of_mass_z = -1., mean_number_of_neighbors = 0.0)
                                
                        cluster_id = 0
                        
                        while self.dropInfo[frame][cluster_id]["size"] > 0:
                                self.G.add_node(
                                        (frame, int(cluster_id)),
                                        size = int(self.dropInfo[frame][cluster_id]['size']),
                                        droplet = -1,
                                        in_flux = 0,
                                        out_flux = 0,
                                        center_of_mass = tuple(self.dropInfo[frame][cluster_id]['center_of_mass']),
                                        mean_number_of_neighbors = self.dropInfo[frame][cluster_id]['mean_number_of_neighbors'],
                                        r_gyr = self.dropInfo[frame][cluster_id]['r_gyr'],
                                        aniso = self.dropInfo[frame][cluster_id]['aniso'])
