...
class SDroplet():
        def __init__(self, droplet_id, tau, nodes, sizes, center_of_mass, local_density):
...
class LifeTime():
        def __init__(self, inputDir):
                print(f"LifeTime : Init {inputDir}")

                self.inputDir = inputDir     
                self.processPath = os.path.join(inputDir, "process.h5")
                
                reader = hdf5Reader(inputDir, "traj.h5")
                self.boxDim = reader.boxDim
                reader.Close()
                
                with h5py.File(self.processPath, 'r') as processFile:

                        self.liqInfo = np.array(cast(h5py.Dataset, processFile["liqInfo"])[:,:,0], dtype=np.int32)
                        self.liqCarac = np.array(cast(h5py.Dataset, processFile["liqInfo"])[:,:,1:], dtype=np.float32)
                        self.dropInfo = np.array(cast(h5py.Dataset, processFile["liqDropInfo"]), dtype=np.float32)
                
                self.N, self.nLiq = np.shape(self.liqInfo)

                self.dropSizeMax = np.max(self.dropInfo[:,0])
                self.dropSizeMin = np.min(np.where(self.dropInfo[:,0] > 0, self.dropInfo[:,0], 2*self.nLiq))
        ...
        @staticmethod
        @jit(nopython=True)
        def centerOfMass(pts : np.ndarray[tuple[int, ...], np.dtype[np.float32]], dims : list[float]) -> tuple[float, float, float]:
                x0, y0, z0 = pts[0]
                
                for i in range(1, len(pts)):
                        x, y, z = pts[i]
                        
                        if abs(x - x0) > dims[0] / 2:
                                x0 = (x0 * i + x - m.copysign(dims[0], x - x0))/(i+1)
                        else:
                                x0 = (x0 * i + x)/(i+1)
                        
                        
                        if abs(y - y0) > dims[1] / 2:
                                y0 = (y0 * i + y - m.copysign(dims[1], y - y0))/(i+1)
                        else:
                                y0 = (y0 * i + y)/(i+1)
                                
                        
                        if abs(z - z0) > dims[2] / 2:
                                z0 = (z0 * i + z - m.copysign(dims[2], z - z0))/(i+1)
                        else:
                                z0 = (z0 * i + z)/(i+1)
                                
                if x0 < 0 or x0 > dims[0]:
                        x0 -= m.copysign(dims[0], x0)
                        
                if y0 < 0 or y0 > dims[1]:
                        y0 -= m.copysign(dims[1], y0)
                        
                if z0 < 0 or z0 > dims[2]:
                        z0 -= m.copysign(dims[2], z0)
        ...
        def Compute_graph(self):
                ...
                for frame in range(1, self.N):
                        if (frame) % 10 == 0:
                                print("LifeTime[Compute_graph] : Process %d out of %d frames" % (frame, self.N))
               
                        nLiq_gaz = int((self.liqInfo[frame] < -.5).sum())
                        
                        if nLiq_gaz > .5:
                                rho_loc_mean = np.mean(self.liqCarac[frame][self.liqInfo[frame] < -.5][:, 3], axis = 0)
                                self.G.add_node((frame, int(-1)), size = nLiq_gaz, droplet = -1, center_of_mass_x = -1., center_of_mass_y = -1., center_of_mass_z = -1., local_density = float(rho_loc_mean))
                                
                        cluster_id = 0
                        
                        while self.dropInfo[frame][cluster_id][0] > 0:
                                rho_loc_mean = np.mean(self.liqCarac[frame][self.liqInfo[frame] == cluster_id][:, 3], axis = 0)
                                cm_x, cm_y, cm_z = self.centerOfMass(self.liqCarac[frame][self.liqInfo[frame] == cluster_id][:, :3], self.boxDim)
        ...
        def Compute_droplet(self):
                ...
                for frame in range(self.N):
                        if (frame) % 10 == 0:
                                print("LifeTime[Compute_droplet] : Process %d out of %d frames" % (frame, self.N))
        ...
        def Check_droplet(self):
                ...
                for keys, droplet in self.droplet_dict.items():
                        ...
                        self.simple_droplet_dict[keys] = SDroplet(droplet.droplet_id, droplet.tau, droplet.nodes, droplet.sizes, droplet.center_of_mass, droplet.local_density)
        ...
        #         exp = self.inputDir.split("data/")[1].split("/")[0]
        #         params = "_".join(self.inputDir.split("data/")[1].split("/N/")[0].split("/")[1:])
        ...
        def Print(self):
                print("\n")
                with open(os.path.join(self.inputDir, "liq_droplets.pickle"), "wb") as pfile:
                        pickle.dump(self.droplet_dict, pfile, protocol=pickle.HIGHEST_PROTOCOL)
                
                with open(os.path.join(self.inputDir, "liq_simple_droplets.pickle"), "wb") as pfile:
