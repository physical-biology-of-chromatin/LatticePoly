...
class ContactHiC():
        ...
        def __init__(self, inputDir, outputDir, binSize, initFrame, cutoff=1/2**0.5 + 1e-3):
                print(f"ContactHiC : Init {inputDir} {outputDir} {binSize} {initFrame}")
        ...
        def Compute(self):
                ...
                self.contactHiC = np.zeros((self.binNum*(self.binNum-1)//2), dtype=np.float32)
                self.contactProb = np.zeros((self.binNum-1), dtype=np.float32)
        ...
        def ProcessFrame(self, i):
                ...
                if i>self.initFrame:
                        ...
                        self._sqDistPBC(polyPos, self.cutoff, self.binNum, self.binSize, self.contactHiC, self.contactProb)
                        

        def Print(self):
                with h5py.File(self.filePath, 'a') as hfile:
                        self.PrintDataset(hfile, "contactProb", self.contactProb)
                        self.PrintDataset(hfile, "contactHiC", self.contactHiC)
        ...
        @staticmethod
        @numba.njit("void(f4[:,:], f4, i4, i4, f4[:], f4[:])")
        def _sqDistPBC(pts, cutoff, binNum, binSize, contactHiC, contactProb):
                cnt = 0
...
if __name__ == "__main__":
        ...
        distMap = ContactHiC(inputDir, outputDir, binSize=binSize, initFrame=initFrame)
