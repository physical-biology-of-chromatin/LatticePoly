...
def merge_matrices(outputDir,name):
	matrices=[]
	for folder in os.listdir(outputDir)[:1]:
		if(folder.endswith('.scool')==False and folder.endswith('.gz')==False and folder.endswith('.res')==False):
			...
			print("startload")
			matrices.append(pd.read_csv(file_path).to_numpy(dtype=np.float))
			print(matrices)
...
#	ser={"chrom":str(int(binsize*9_000))}
