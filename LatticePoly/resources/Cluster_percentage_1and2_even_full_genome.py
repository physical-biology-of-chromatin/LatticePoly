...
class cluster_full_genome():
	def __init__(self, outputDir, initFrame):
		...
		self.Forks_per_clusterFile = os.path.join(outputDir,str(r)+"_"+str(initFrame)+"_"+str(interval)+"_forks_"+str(forks_n)+"percentage(1and2forks)_"+str(percentage)+"_cluster_even_full_genome.res")
	...
	def Print(self):
		...
		print(self.forks_per_cluster[:10])

if __name__ == "__main__":
	if len(sys.argv) != 7:
		print("\033[1;31mUsage is %s outputDir initFrame r interval fork_n percentage(non-detected small forks) \033[0m" % sys.argv[0])
		sys.exit()
