...
class DistanceMap():
	...
	def __init__(self, outputDir, initFrame, nStride, printAllFrames=True, cutoff=1/2**0.5 + 1e-3):
		...
		self.nBins = self.reader.Nchain // self.nStride
		self.nPrune = self.reader.Nchain % self.nStride
	...
	def ProcessFrame(self, i):
		...
		#here nbin has to be different->self.reader.Ntad
	...
	def _sqDistPBC(dims, pts, types, cutoffs, contHist, sqDist):
		cnt = 0
		#put condition if len pts is greater than Nbin
		nPoints = pts.shape[0]
