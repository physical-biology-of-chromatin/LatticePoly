...
class RepliDistanceMap():
	...
	def __init__(self, outputDir, initFrame, finalFrame, threshold=0.5, nMax=10, cutoff=1/2**0.5 + 1e-3):
		...
		self.CisMapFile = os.path.join(self.reader.outputDir,str(time.time())+ "CisMap.res")
		self.TransMapFile = os.path.join(self.reader.outputDir,str(time.time())+ "TransMap.res")
		self.AllFile = os.path.join(self.reader.outputDir,str(time.time())+ "All.res")
