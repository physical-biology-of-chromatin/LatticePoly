...
class Droplet():
        def __init__(self, droplet_id, node, size, in_event):
        ...
        def append(self, node, size, in_event):
...
class LifeTime():
        def __init__(self, outputDir, initFrame):
                ...
                self.liqInfo = self.file["liqInfo"]
                self.dropInfo = self.file["liqDropInfo"]
                ...
                self.node_per_frame = {frame : [] for frame in range(self.initFrame+2, self.N)}
                self.droplet_dict = {}
                self.droplet_id = 0
        ...
        def Compute_graph(self):
                ...
                for frame in range(self.initFrame+2, self.N):

                        nLiq_gaz = int((1 - self.file["liqFraction"][frame])*24**3*4)
                        if nLiq_gaz != 0:
                                self.G.add_node((frame, int(-1)), size = nLiq_gaz, droplet = -1)
                                self.node_per_frame[frame].append((frame, int(-1)))

                        cluster_id = 0
                        while self.dropInfo[frame][cluster_id][0] > 0:
                                self.G.add_node((frame, int(cluster_id)), size = int(self.dropInfo[frame][cluster_id][0]), droplet = -1, in_flux = 0, out_flux = 0)#, r_gyr = self.dropInfo[frame][cluster_id][1], aniso = self.dropInfo[frame][cluster_id][2])


                                cluster_id += 1
                        if frame > 1:
                                for i in range(self.nLiq):
                                        try:
                                                self.G.edges[(frame-1, int(self.liqInfo[frame-1][i])),(frame, int(self.liqInfo[frame][i]))]["weight"] += 1
                                        except:
                                                self.G.add_edge((frame-1, int(self.liqInfo[frame-1][i])),(frame, int(self.liqInfo[frame][i])), event = "")
                                                self.G.edges[(frame-1, int(self.liqInfo[frame-1][i])),(frame, int(self.liqInfo[frame][i]))]["weight"] = 1
                                        if int(self.liqInfo[frame-1][i]) < -.5 and int(self.liqInfo[frame][i]) > -.5:
                                                self.G.nodes[(frame,int(self.liqInfo[frame][i]))]["in_flux"] += 1
                                        if int(self.liqInfo[frame-1][i]) > -.5 and int(self.liqInfo[frame][i]) < -.5:
                                                self.G.nodes[(frame-1,int(self.liqInfo[frame-1][i]))]["out_flux"] += 1

                        elif frame > 0:
                                for i in range(self.nLiq):
                                        try:
                                                self.G.edges[(0, -1),(frame, int(self.liqInfo[frame][i]))]["weight"] += 1
                                        except:
                                                self.G.add_edge((0, -1),(frame, int(self.liqInfo[frame][i])))
                                                self.G.edges[(0, -1),(frame, int(self.liqInfo[frame][i]))]["weight"] = 1
                                        if int(self.liqInfo[frame][i]) > -.5:
                                                self.G.nodes[(frame,int(self.liqInfo[frame][i]))]["in_flux"] += 1
                ...
                self.dict_max_receive = {(frame, cluster) : max({(neigh_frame, neigh_cluster) : weight if neigh_frame < frame else -1 for _, (neigh_frame, neigh_cluster), weight in self.G.edges((frame, cluster), data = "weight")}.items(), key=lambda k: k[1])[0] if frame < self.N-1 else (frame, cluster) for frame, cluster in self.G.nodes()}

                self.dict_edges_per_frame_and_type = {(frame, frame+1) : {0 : [], 'A' : [], 'B' : [], 'C' : []} for frame in range(self.initFrame+1, self.N-1)}
        ...
        def Compute_droplet(self):
                ...
                for frame in range(self.initFrame+1, self.N-2):
        ...
        def Compute_edge_0(self, edge):
                ...
                if u[1] < -.5 and list_in_v_B == []: # Cas 1 : Emergence
                        self.droplet_dict[self.droplet_id] = Droplet(self.droplet_id, v, self.G.nodes[v]["size"], [Event('emergence', self.G.nodes[v]["in_flux"], None)])
                        ...
                        self.droplet_dict[self.G.nodes[u]["droplet"]].append(v, self.G.nodes[v]["size"], [Event('persistance', self.G.edges[v, u]["weight"], u )])
        ...
        def Compute_edge_A(self, edge):
                ...
                if self.G.edges[edge]['event'] == "":
                        ...
                        if u[1] < -.5 and list_in_v_B != []: # Cas 2 : persistance en cas de expand
                                ...
                                self.droplet_dict[self.G.nodes[in_v_B_max]["droplet"]].append(v, self.G.nodes[v]["size"], [Event('persistance', self.G.edges[v, in_v_B_max]["weight"], in_v_B_max)])
                                ...
                                if self.dict_max_give[u][1] < -.5: # g donne son maximum de particules au flux donc persistence ou splinter
                                        ...
                                        if out_node_B_max == v:         # Cas de persistence
                                                ...
                                                self.droplet_dict[self.droplet_id] = Droplet(self.droplet_id, v, self.G.nodes[v]["size"], [Event('splinter', self.G.edges[v, u]["weight"], u)])
                                        ...
                                        self.droplet_dict[self.droplet_id] = Droplet(self.droplet_id, v, self.G.nodes[v]["size"], [Event('split', self.G.edges[v, u]["weight"], u)])
                                        ...
                                        self.droplet_dict[self.droplet_id] = Droplet(self.droplet_id, v, self.G.nodes[v]["size"], [Event('offload', self.G.edges[v, u]["weight"], u)])
        ...
        def Print_film(self):
                self.reader = hdf5Reader(self.outputDir, "traj.h5", initFrame, readLiq=True, readPoly=False, backInBox=False)
        ...
        def Print(self):
                ...
                fig.savefig(f"/home/ppuel/data/{exp}/LifeTime_{params}.png")
        ...
        def Print_data(self):
                ...
                nx.write_gml(self.G, os.path.join(self.outputDir, "liq_graph"))
...
if __name__ == "__main__":
        ...
        A = time.time()
        lifeTime = LifeTime(outputDir, initFrame=initFrame)
        B = time.time()
