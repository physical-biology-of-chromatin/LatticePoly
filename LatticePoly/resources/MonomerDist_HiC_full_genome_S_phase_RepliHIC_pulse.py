...
class MonomerDmap():
	def __init__(self, outputDir, initFrame):
		self.readers=[]
		self.N_chain=[]
		pulsetime=random.randint(initFrame,initFrame+900-2*interval-1)
		for i in range(0,17):
			self.readers.append(vtkReader(outputDir, i,pulsetime,readLiq=False, readPoly=True))
			self.N_chain.append(np.sum([(self.readers[i].status==-1)+(self.readers[i].status==0)]))
		self.contactFile = os.path.join(outputDir,"r_"+str(r)+"_"+str(initFrame)+ "_"+str(interval)+"full_genome_repliHiC_pulse_unique.cool")
		self.brdu_mask=np.zeros(2*np.sum(self.N_chain))

		self.Compute(2*interval)
...
#self.contactProb=np.rint(self.contactProb/finalFrame)
	...
	def ProcessFrame(self, i):
		...
		for chrom,reader in enumerate(self.readers):
			...
			forks_chrom=np.full(2*self.N_chain[chrom],0)

			for pos_id,pos in enumerate(data.polyPos):
				if(pos_id<self.N_chain[chrom]):
					positions_sister[pos_id]=pos
					forks_chrom[pos_id]=data.fork[pos_id]
				else:
					positions_sister[self.N_chain[chrom]+data.SisterID[pos_id]]=pos
					
			full_positions.append(positions_sister)
			full_forks.append(forks_chrom)
		full_forks=np.concatenate(full_forks)
		if(i<interval):
			self.brdu_mask[full_forks!=0]+=1
		if(i>=interval):
			tree1	= cKDTree(np.concatenate(full_positions), boxsize = None)
			pairs = list(tree1.query_pairs(r = r*0.71)) # NN distance FCC lattice 1/np.sqrt(2) = 0.71
			sh_indices=np.arange(len(pairs))
			random.shuffle(sh_indices)
			shuffled_pairs=[pairs[i] for i in sh_indices]
			...
			for (i, j) in shuffled_pairs:
				if i in used or j in used:
					continue
				if self.brdu_mask[i] != 0 or self.brdu_mask[j] != 0:
