...
class vtkReader():
    ...
    def InitReader(self, initFrame):
        try:
            ...
            print("Box linear dimensions: (%.0f,%.0f,%.0f)" %
                  tuple(self.boxDim))
            ...
            if self._readPoly:
                ...
                print("Initial chromatin state: %d TADs inc. %d heterochromatic loci" % (
                    self.nTad, self.nHet))
    ...
    def _readLiqFrame(self):
        ...
        self.liqDens = vn.vtk_to_numpy(
            liqData.GetPointData().GetArray("Density"))
        self.liqDisp = vn.vtk_to_numpy(
            liqData.GetPointData().GetArray("Displacement"))
    ...
    def _readPolyFrame(self):
        ...
        self.domains = np.split(hetDomains, np.where(
            np.diff(hetDomains) != 1)[0] + 1)
    ...
    def _checkRange(self, initFrame):
        ...
        if (self._readPoly & self._readLiq):
            ...
            if not minFrame <= self.initFrame <= maxFrame:
                raise IOError("Frame not in range (%d, %d)" %
                              (minFrame, maxFrame))
            ...
            if not self._minFramePoly <= self.initFrame <= self._maxFramePoly:
                raise IOError("Frame not in range (%d, %d)" %
                              (self._minFramePoly, self._maxFramePoly))
            ...
            if not self._minFrameLiq <= self.initFrame <= self._maxFrameLiq:
                raise IOError("Frame not in range (%d, %d)" %
                              (self._minFrameLiq, self._maxFrameLiq))
    ...
    def _parseFileSeqs(self):
        if self._readPoly:
            try:
                ...
                raise IOError(
                    "Could not locate any polymer configuration files in '%s'" % self.outputDir)
        ...
        if self._readLiq:
            try:
                ...
                raise IOError(
                    "Could not locate any liquid configuration files in '%s'" % self.outputDir)
