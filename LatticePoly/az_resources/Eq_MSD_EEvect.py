...
class msdEE():
    ...
    def Compute(self, domainlen):
        ...
        for j in range(self.reader.nTad - 3*domainlen):    
            ...
            self.distTad += np.sum((PosHist - PosHist[0])*(PosHist - PosHist[0]), axis = 1)   
    ...
    def ComputeDomainend(self, domainstart, domainend):
        ...
        self.msdEndtoEnd = np.sum((PosHist - PosHist[0])*(PosHist - PosHist[0]), axis = 1) 

    def PrintDomainlen(self):
        #np.savetxt(self.anisoFile, self.polyAniso)
        np.savetxt(self.endtoendFile, self.distTad)

        print("\033[1;32mPrinted msd of end to end vector (domain averaged) NOT time averaged to '%s'\033[0m" %self.endtoendFile)
    ...
    def PrintDomainend(self):
            ...
            print("\033[1;32mPrinted msd of end to end vector (NOT time averaged) to '%s'\033[0m" %self.endtoendFile)
            #print("\033[1;32mPrinted domain anisotropy factors to '%s'\033[0m" % self.anisoFile)
